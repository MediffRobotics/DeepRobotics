...
% Last Modified by GUIDE v2.5 29-Apr-2016 22:00:19
...
global AllMovements;
global MaxIndex_AllMovements;
MaxIndex_AllMovements=0;
AllMovements={};
finishCodition='No';
UserIn='No';

while(isequal(UserIn,finishCodition))
    MaxIndex_AllMovements=MaxIndex_AllMovements+1;
    [fileName,filePath]=uigetfile('*.mat','Choose a .mat data');
    load(fileName);
    AllMovements{MaxIndex_AllMovements}=movementsRecording;
    UserIn=questdlg('finish to load all movements？');
...
for i=1:MaxIndex_AllMovements
    CurrentMovements=AllMovements{i};
    
    % getting the longest length moves
    [~,size_f]=size(CurrentMovements{'f'});
    [~,size_g]=size(CurrentMovements{'g'});
    [~,size_h]=size(CurrentMovements{'h'});
    max_size_moves=max([size_f size_g size_h]);
    for j=1:max_size_moves   % For each movement frame
        % Loading movement data
         if(j<=size_f)
            numDataAvai=numDataAvai+1; 
            MovementsForJoint_f=CurrentMovements{'f'}(j);
            SendingToSerial( MovementsForJoint_f,'f',handles );
        end
        
        if(j<=size_g)
             numDataAvai=numDataAvai+1;
            MovementsForJoint_g=CurrentMovements{'g'}(j);
            SendingToSerial( MovementsForJoint_g,'g',handles);
        end
        
        if(j<=size_h)
            numDataAvai=numDataAvai+1;
            MovementsForJoint_h=CurrentMovements{'h'}(j);
        ...
        %% wait for finish
        k=1;                
          while(ReceivedTimesFromSerial<numDataAvai)
            k=k+1;
            
            pause(0.01); % wait for a while
            if(k==10000)
                'time out'
                k==0;
                break;
...
if isequal('q',ReceivedCharacter)
    str_stepsVal =MovementRecording(get(handles.SLIDER_NumSteps,'value'),'a',1);    
    str_stepsVal=1;
     fprintf(handles.serial,'%s',str_stepsVal);      
end
...
if isequal('w',ReceivedCharacter)
    str_stepsVal =MovementRecording(get(handles.SLIDER_NumSteps,'value'),'b',1);    
     fprintf(handles.serial,'%s',str_stepsVal);      
end
...
if isequal('e',ReceivedCharacter)
    str_stepsVal =MovementRecording(get(handles.SLIDER_NumSteps,'value'),'c',1);    
     fprintf(handles.serial,'%s',str_stepsVal);      
end
...
if isequal('r',ReceivedCharacter)
    str_stepsVal =MovementRecording(get(handles.SLIDER_NumSteps,'value'),'e',1);   
     fprintf(handles.serial,'%s',str_stepsVal);      
end
...
if isequal('t',ReceivedCharacter)
    str_stepsVal =MovementRecording(get(handles.SLIDER_NumSteps,'value'),'f',1);    
     fprintf(handles.serial,'%s',str_stepsVal);      
end
...
if isequal('y',ReceivedCharacter)
    str_stepsVal =MovementRecording(get(handles.SLIDER_NumSteps,'value'),'g',1);    
     fprintf(handles.serial,'%s',str_stepsVal);      
end
...
if isequal('u',ReceivedCharacter)
    str_stepsVal =MovementRecording(get(handles.SLIDER_NumSteps,'value'),'h',1);    
     fprintf(handles.serial,'%s',str_stepsVal);      
end
