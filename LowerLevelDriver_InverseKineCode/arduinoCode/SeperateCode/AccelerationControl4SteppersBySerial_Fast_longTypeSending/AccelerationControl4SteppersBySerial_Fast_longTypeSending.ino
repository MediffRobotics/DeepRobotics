...
#include <Servo.h>
#include <math.h>
//Speed controllig flag
boolean ProtectingSpeed[4]={0,0,0,0};
...
ISR(TIMER2_COMPA_vect)
{
 ...
 //Checking to reset speed
 counter++;
 if(ProtectingSpeed[0]==0&&ProtectingSpeed[1]==0&&ProtectingSpeed[2]==0&&ProtectingSpeed[3]==0){
  
   /*
   if(counter%100==0 &&OCR2A>30){
   OCR2A = OCR2A-1;
   }
   */
   OCR2A=30;
  ...
  Acceleration_ElbowRotateGivenSteps_ISR();
  Acceleration_AxisRotateStepper_ISR();
  Acceleration_UpperArmStepper_ISR();
 Acceleration_ROTATE_BASEStepper_ISR();
...
/////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////
unsigned int decelerateStepPoint[5]={1000,2000,4000,6000,9000};
unsigned int speedBase=10;

unsigned int Acceleration_Alg(unsigned int TargetSteps,unsigned int CurrentSteps, long CurrentTargetStepDiff){
  
      //Check if need to reset speed
      //OCR2A = 30;           
      if(TargetSteps<=decelerateStepPoint[4]){
        return 4;
      }
      
      
      //Accelerating   
      if(CurrentSteps<=decelerateStepPoint[0]){
        return 9;
      }
     else if(CurrentSteps<=decelerateStepPoint[1]){
        return 5;
      }
    else if(CurrentSteps<=decelerateStepPoint[2]){
        return 3;
      }
    else if(CurrentSteps<=decelerateStepPoint[3]){
        return 2;
      }
    else if(CurrentSteps<=decelerateStepPoint[4]){
        OCR2A = 30+(decelerateStepPoint[4]-CurrentSteps)/100; 
        return 1;
      }
    ...
    //Decelerating 
     if(CurrentTargetStepDiff<=decelerateStepPoint[0]){
        return 9;
        }  
     else if(CurrentTargetStepDiff<=decelerateStepPoint[1]){
       return 6;
     }   
     else if(CurrentTargetStepDiff<=decelerateStepPoint[2]){
       return 4;
     }  
     
     else if(CurrentTargetStepDiff<=decelerateStepPoint[3]){
       return 2;
       
     }  
     else if(CurrentTargetStepDiff<=decelerateStepPoint[4]){        
       OCR2A = 30+((decelerateStepPoint[4]-(TargetSteps-CurrentSteps))/100);          
       return 1;               
     }       
      else{
        return 1;
...
void Acceleration_ROTATE_BASEStepper_ISR(){
  //Init decelerating variables
  static unsigned int PeriodChanging=1; 
  static unsigned int speedIndex=1; 
  //Checking if need to reset speed
  unsigned long CurrentTargetStepsDiffer=ROTATE_BASEStepper_TotalSteps-StepCounter_ROTATE_BASE;
  
  if((StepCounter_ROTATE_BASE<=9000&&StepCounter_ROTATE_BASE>=6000 )||
  (CurrentTargetStepsDiffer<=9000 && CurrentTargetStepsDiffer>=6000))
  {                     // need to change speed
  ProtectingSpeed[0]=1;
  }
  else {                    // need to reset speed
  ProtectingSpeed[0]=0;
  }
  
  PeriodChanging=Acceleration_Alg(ROTATE_BASEStepper_TotalSteps,StepCounter_ROTATE_BASE,CurrentTargetStepsDiffer);      
  if(speedIndex%PeriodChanging==0){
...
void Acceleration_UpperArmStepper_ISR(){
  ...
  long CurrentTargetStepsDiffer=uparmStepper_TotalSteps-StepCounter_UPARM;
  
  if((StepCounter_UPARM<=9000&&StepCounter_UPARM>=6000 )||
  (CurrentTargetStepsDiffer<=9000 && CurrentTargetStepsDiffer>=6000))
  {                     // need to change speed
  ProtectingSpeed[1]=1;
  }
  ...
  PeriodChanging=Acceleration_Alg(uparmStepper_TotalSteps,StepCounter_UPARM,CurrentTargetStepsDiffer);  
  if(speedIndex%PeriodChanging==0){
    UpperArmStepper_ISR();    
  }
  speedIndex++;
...
void Acceleration_ElbowRotateGivenSteps_ISR(){
  //Init decelerating variables
  static unsigned int PeriodChanging=1; 
  static unsigned int speedIndex=1; 
  
  long CurrentTargetStepsDiffer=elbowStepper_TotalSteps-StepCounter_ELBOW;
  
  if((StepCounter_ELBOW<=9000&&StepCounter_ELBOW>=6000 )||
  (CurrentTargetStepsDiffer<=9000 && CurrentTargetStepsDiffer>=6000))
  {                     // need to change speed
  ProtectingSpeed[2]=1;
  }
  ...
  PeriodChanging=Acceleration_Alg(elbowStepper_TotalSteps,StepCounter_ELBOW,CurrentTargetStepsDiffer);  
  if(speedIndex%PeriodChanging==0){
...
void Acceleration_AxisRotateStepper_ISR(){
  //Init decelerating variables
  static unsigned int PeriodChanging=1; 
  static unsigned int speedIndex=1; 
  
  long CurrentTargetStepsDiffer=AxisRotate_TotalSteps-StepCounter_AXIS;
  
  if((StepCounter_AXIS<=9000&&StepCounter_AXIS>=6000 )||
  (CurrentTargetStepsDiffer<=9000 && CurrentTargetStepsDiffer>=6000))
  {                     // need to change speed
  ProtectingSpeed[3]=1;
  }
  ...
  PeriodChanging=Acceleration_Alg(AxisRotate_TotalSteps,StepCounter_AXIS,CurrentTargetStepsDiffer);  
  if(speedIndex%PeriodChanging==0){
