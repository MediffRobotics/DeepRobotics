...
/************Wrist1 STEPPER SETUP****************/
/***************************************************/

// To indicate if stepper is working. 
// If yes, True ; otherwise, False

volatile  unsigned long StepCounter_Wrist1=0;
volatile bool DirectionFlag_Wrist1=0;
volatile  unsigned long Wrist1Stepper_TotalSteps=0;
volatile bool Wrist1Stepper_WorkingFlag=false;
volatile  static bool sentFlag_Wrist1=false; //checking sent signal


const int CP_Wrist1  =  5;      
const int DIR_Wrist1 =  4;      
const int Task_ENA_Wrist1 =  13;
const int Task_ENA_Wrist1_2 =  6;
const int AutoCaliButtonPin = 3;     // the number of the pushbutton pin

// setting Wrist1 stepper driver
void setup_Wrist1Stepper(){
  pinMode(Task_ENA_Wrist1, OUTPUT);
  pinMode(Task_ENA_Wrist1_2, OUTPUT);
  
  pinMode(DIR_Wrist1, OUTPUT);  
  pinMode(CP_Wrist1,OUTPUT);
  pinMode(AutoCaliButtonPin, INPUT);  
   
  digitalWrite(Task_ENA_Wrist1, HIGH);  
  digitalWrite(Task_ENA_Wrist1_2, HIGH); 
  }
  
  
void UpperArmGivenSteps( unsigned long steps,bool Step_direction){  
  
  digitalWrite(Task_ENA_Wrist1, LOW); //Enable sign
  digitalWrite(Task_ENA_Wrist1_2, LOW); //Enable sign
  Wrist1Stepper_WorkingFlag=true;
  DirectionFlag_Wrist1=Step_direction; 
  Wrist1Stepper_TotalSteps=steps+Wrist1Stepper_TotalSteps;      
  }
    ...
    if(digitalRead(AutoCaliButtonPin)==HIGH)
  {
    ...
    sentFlag_Wrist1=true;    
   //Reset parameters
    StepCounter_Wrist1=0;    
    Wrist1Stepper_TotalSteps=0;
    Wrist1Stepper_WorkingFlag=false;
  }
  ...
  digitalWrite(DIR_Wrist1, DirectionFlag_Wrist1);  //Determin direction
  
  //AutoCalibration();
  
  //determing the task is finished,yes,reset variables
  if((StepCounter_Wrist1==Wrist1Stepper_TotalSteps)||(Wrist1Stepper_WorkingFlag==false)){        
   if((StepCounter_Wrist1==Wrist1Stepper_TotalSteps)&&(Wrist1Stepper_TotalSteps!=0)){
   sentFlag_Wrist1=true;
   }
  ...
  Wrist1Stepper_WorkingFlag=true; // to indicate the stepper is still working   
  StepCounter_Wrist1++; 
  digitalWrite(CP_Wrist1,outputPulse);
...
void Acceleration_UpperArmStepper_ISR(){
  unsigned long CurrentTargetStepsDiffer=Wrist1Stepper_TotalSteps-StepCounter_Wrist1;
  Acceleration_Alg(Wrist1Stepper_TotalSteps,StepCounter_Wrist1,CurrentTargetStepsDiffer);  
  UpperArmStepper_ISR();    
}
...
void StepperWorkingCheck(){
   ...
   if(sentFlag_Wrist1==true){
     sentFlag_Wrist1=false;     
     digitalWrite(Task_ENA_Wrist1, HIGH);
     digitalWrite(Task_ENA_Wrist1_2, HIGH);
   } 
