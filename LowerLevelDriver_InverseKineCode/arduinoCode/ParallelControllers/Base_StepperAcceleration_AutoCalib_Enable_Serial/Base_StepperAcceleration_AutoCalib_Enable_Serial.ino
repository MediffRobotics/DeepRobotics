...
unsigned long decelerateStepPoint=5000;
...
const uint8_t myTOP = 15;//max speed=15,
void setMyTimer2( ){
  cli();  // 禁止中斷
...
volatile  unsigned long StepCounter_Base=0;
volatile bool DirectionFlag_Base=0;
volatile  unsigned long BaseStepper_TotalSteps=0;
volatile bool BaseStepper_WorkingFlag=false;
volatile  static bool sentFlag_Base=false; //checking sent signal


const int CP_Base  =  5;      
const int DIR_Base =  4;      
const int Task_ENA_Base =  13;
const int Task_ENA_Base2 =  6;
const int AutoCaliButtonPin = 3;     // the number of the pushbutton pin

// setting Base stepper driver
void setup_BaseStepper(){
  pinMode(Task_ENA_Base, OUTPUT);
  pinMode(Task_ENA_Base2, OUTPUT);
  pinMode(DIR_Base, OUTPUT);  
  pinMode(CP_Base,OUTPUT);
  pinMode(AutoCaliButtonPin, INPUT);  
   
  digitalWrite(Task_ENA_Base, HIGH);
digitalWrite(Task_ENA_Base2, HIGH);  
  }
  
  
void BaseGivenSteps( unsigned long steps,bool Step_direction){  
  
  digitalWrite(Task_ENA_Base, LOW); //Enable sign
   digitalWrite(Task_ENA_Base2, LOW); //Enable sign
  BaseStepper_WorkingFlag=true;
  DirectionFlag_Base=Step_direction; 
  BaseStepper_TotalSteps=steps+BaseStepper_TotalSteps;      
  }
    ...
    if(digitalRead(AutoCaliButtonPin)==HIGH)
  {
   ...
   //Reset parameters
    StepCounter_Base=0;    
    BaseStepper_TotalSteps=0;
    BaseStepper_WorkingFlag=false;
  }
  
  
  }
    
  void BaseStepper_ISR(){
  static boolean outputPulse=HIGH;  
  digitalWrite(DIR_Base, DirectionFlag_Base);  //Determin direction
  
  //AutoCalibration();
  
  //determing the task is finished,yes,reset variables
  if((StepCounter_Base==BaseStepper_TotalSteps)||(BaseStepper_WorkingFlag==false)){        
   if((StepCounter_Base==BaseStepper_TotalSteps)&&(BaseStepper_TotalSteps!=0)){
   sentFlag_Base=true;
   }
  ...
  BaseStepper_WorkingFlag=true; // to indicate the stepper is still working   
  StepCounter_Base++; 
  digitalWrite(CP_Base,outputPulse);
...
void Acceleration_BaseStepper_ISR(){
  unsigned long CurrentTargetStepsDiffer=BaseStepper_TotalSteps-StepCounter_Base;
  Acceleration_Alg(BaseStepper_TotalSteps,StepCounter_Base,CurrentTargetStepsDiffer);  
  BaseStepper_ISR();    
}
...
void ControlSteppersBySerial(){  
      ...
      BaseGivenSteps(numSteps,s_direction);
