...
% Last Modified by GUIDE v2.5 11-Jan-2017 12:02:50
...
[~,size_f]=size(CurrentMovements{'f'-MotorTagBase}); 
    ...
    for MoveNumIndex=1:size_f   % For each movement frame
       ...
       MovementsForJoint_a=[num2str(CurrentMovements{'a'-MotorTagBase}(MoveNumIndex)) 'a,'];
      MovementsForJoint_b=[num2str(CurrentMovements{'b'-MotorTagBase}(MoveNumIndex)) 'b,'];
       MovementsForJoint_c=[num2str(CurrentMovements{'c'-MotorTagBase}(MoveNumIndex)) 'c,'];
      MovementsForJoint_d=[num2str(CurrentMovements{'d'-MotorTagBase}(MoveNumIndex)) 'd,'];
      MovementsForJoint_e=[num2str(CurrentMovements{'e'-MotorTagBase}(MoveNumIndex)) 'e,'];
      MovementsForJoint_f=[num2str(CurrentMovements{'f'-MotorTagBase}(MoveNumIndex)) 'f;'];
...
SockReturnArmToOrigion( handles );
% sending finish flag
SendingBySocketInRaw( 'finish;')
...
if isequal('r',ReceivedCharacter)
    str_stepsVal =MovementRecording(NumStepsEachTime,'D',1,MoveIndex);   
...
%% Sending to socket
%str_stepsVal='1000A';
SendingDataToArmBySocket(str_stepsVal);

['Sending ' str_stepsVal]
%sending to gui
%str2 = ['Data has sent sucessfully!'];
%set(handles.text3,'String',str2,'fontweight','bold');
...
% --- Executes on button press in pushbutton_OpenSocket.
function pushbutton_OpenSocket_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_OpenSocket (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

%% client declear 
global sockCommu;
sockCommu = tcpip('169.254.138.189', 30005, 'NetworkRole', 'client');
fopen(sockCommu)
%% server declear 
global sockServer
sockServer= tcpip('169.254.138.189', 3004, 'NetworkRole', 'server');
fopen(sockServer);


% --- Executes on button press in pushbutton_CloseSocket.
function pushbutton_CloseSocket_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_CloseSocket (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global sockCommu;
fclose(sockCommu)
...
% --- Executes on button press in StartMoving.
function StartMoving_Callback(hObject, eventdata, handles)
% hObject    handle to StartMoving (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in LoopMotion.
function LoopMotion_Callback(hObject, eventdata, handles)
% hObject    handle to LoopMotion (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Otherwise, executes on mouse press in 5 pixel border or over pushbutton_OpenSocket.
function pushbutton_OpenSocket_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to pushbutton_OpenSocket (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
